...
export class ChatService {
  ...
  constructor() {
    // Ensure apiKey is a string to prevent n.indexOf errors in the SDK
  ...
  public async generateResponse(history: Message[], config: ChatConfig, systemInstructionOverride?: string): Promise<string> {
    const contents = history.map(msg => ({
      role: msg.role === 'user' ? 'user' : 'model',
      parts: [{ text: String(msg.text || '') }]
    }));
    ...
    try {
      const response = await this.ai.models.generateContent({
        ...
        contents: contents,
        config: {
          systemInstruction: systemInstructionOverride || config.systemInstruction,
          temperature: config.temperature || 0.8,
          topP: config.topP || 0.9,
          topK: config.topK || 40,
        }
      ...
      return `Neural transmission failed: ${error.message}`;
    }
  }
}
