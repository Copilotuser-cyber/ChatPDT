...
interface CommunityPageProps {
  onClose: () => void;
  currentUser: User;
  onRemix?: (game: GameProject) => void;
}

export const CommunityPage: React.FC<CommunityPageProps> = ({ onClose, currentUser, onRemix }) => {
                  ...
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden border-2 dark:border-slate-700 shadow-sm">
                      {post.profilePic ? (
                        ...
                        <span className="text-xs font-black text-indigo-500">{String(post.username || 'A').charAt(0).toUpperCase()}</span>
                      )}
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[11px] font-black dark:text-white uppercase tracking-wider">{post.username || 'Anonymous'}</span>
             ...
             <header className="flex justify-between items-center mb-8">
               ...
               <div className="flex gap-4">
                 <button 
                   ...
                   className="px-8 py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-600/20"
                 >
                   Remix Signal
                 </button>
