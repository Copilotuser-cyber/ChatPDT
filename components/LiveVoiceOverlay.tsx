  ...
  useEffect(() => {
    const startLiveSession = async () => {
      try {
        ...
        // connectLive is now implemented in geminiService to proxy the underlying SDK call
        const sessionPromise = geminiService.connectLive({
          // GUIDELINE: Real-time audio & video conversation tasks: 'gemini-2.5-flash-native-audio-preview-12-2025'
    ...
    <div className="fixed inset-0 z-[150] flex flex-col items-center justify-center bg-slate-950/95 backdrop-blur-xl animate-fade-in">
      <div className="relative flex flex-col items-center max-w-md w-full p-8 text-center">
        ...
        <h2 className="text-2xl font-black text-white uppercase tracking-[0.2em] mb-2">
          {status === 'connecting' ? 'Initializing...' : status === 'listening' ? 'System Listening' : status === 'speaking' ? 'Gemini Speaking' : 'Interface Error'}
        </h2>
        
        <p className="text-slate-400 text-sm mb-12 font-medium">
          {status === 'listening' ? 'Speak naturally. Gemini will respond instantly.' : 
           status === 'speaking' ? 'Gemini is processing and responding via voice link.' :
           status === 'error' ? errorMsg : 'Establishing neural voice connection...'}
        </p>
      ...
      <div className="absolute bottom-8 flex items-center gap-3 opacity-30">
        ...
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Quantum Voice Decryption Active</span>
        <div className="h-[1px] w-8 bg-slate-500"></div>
      </div>
    </div>
  );
