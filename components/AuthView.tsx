  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#0f172a] relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-800/50 border border-slate-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all placeholder:text-slate-600"
            ...
            {error && <div className="text-red-400 text-xs text-center font-medium animate-shake">{error}</div>}

            <button 
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-indigo-500/20 transition-all active:scale-95"
            >
        ...
        <p className="text-center text-slate-600 text-[10px] mt-8 uppercase tracking-widest opacity-50">Secure PDT Protocol Enabled</p>
      </div>
    </div>
  );
};
