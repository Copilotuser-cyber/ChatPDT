        ...
        <div className="w-80 border-r dark:border-slate-800 overflow-y-auto p-6 space-y-3 bg-slate-50/50 dark:bg-slate-900/20 custom-scrollbar">
          ...
          {users.map(u => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-xs font-black shadow-inner transition-all overflow-hidden ${selectedUser?.id === u.id ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 group-hover:bg-indigo-500/10'}`}>
                  {u.profilePic ? (
          ...
          {selectedUser ? (
            <>
              <div className="p-6 border-b dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center gap-4">
                <div className="w-8 h-8 rounded-xl bg-indigo-500 flex items-center justify-center text-white text-[10px] font-bold shadow-md overflow-hidden">
