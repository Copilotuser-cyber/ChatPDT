...
import { Message, Role, ChatConfig, User, Theme } from './types';
...
const App: React.FC = () => {
  ...
  // Load user from session if needed (simulated)
  useEffect(() => {
  ...
  const handleClearChat = () => {
    if (currentUser) {
      storage.clearUserMessages(currentUser.id);
      setMessages([]);
      initChat();
    }
  };
  ...
  return (
    <div className={`flex h-screen w-full overflow-hidden ${theme}`}>
      <div className="flex h-full w-full dark:bg-slate-950 bg-white">
        <div className="lg:hidden absolute top-0 left-0 right-0 h-16 dark:bg-slate-900 bg-white dark:border-slate-800 border-slate-200 border-b flex items-center justify-between px-4 z-40 transition-colors">
          ...
          <div className="w-10 h-10 rounded-full dark:bg-slate-800 bg-slate-100 flex items-center justify-center font-bold text-indigo-500">
        ...
        <main className="flex-1 flex flex-col min-w-0 h-full lg:pt-0 pt-16 relative">
          <header className="hidden lg:flex h-16 items-center justify-between px-8 border-b dark:border-slate-900 border-slate-200 dark:bg-slate-950/50 bg-white/50 backdrop-blur-xl z-10 transition-colors">
             <div className="flex items-center gap-4">
                ...
                <h1 className="text-sm font-black dark:text-white text-slate-800 uppercase tracking-[0.3em]">Neural Interface</h1>
             </div>
             <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                   <span className="text-[10px] font-bold dark:text-emerald-500/70 text-emerald-600 uppercase tracking-widest">System Ready</span>
                </div>
             </div>
          </header>
